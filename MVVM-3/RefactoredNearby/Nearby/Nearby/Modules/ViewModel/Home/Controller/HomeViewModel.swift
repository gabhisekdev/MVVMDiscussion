...
class HomeViewModel {
    ...
    private var webServiceManager: WebServiceManagerProtocol!
    ...
    private func getAppData(completion: @escaping ()->()) {
        ...
        for placeType in allPlaces {
            PlaceWebService().getPlaceList(manager: webServiceManager,placeType: placeType) { (placeList, error) in
                if placeList != nil {
                    places+=placeList!
                }
                dataReceivedCount+=1
    ...
    /// Provides a categoriesCell type.
